...
-- =====================================================
CREATE TABLE IF NOT EXISTS knowledge.indexed_documents (
    -- Identity
...
CREATE INDEX IF NOT EXISTS idx_indexed_docs_stale_pending
    ON knowledge.indexed_documents(status, created_at)
    WHERE status = 'PENDING';

-- =====================================================
-- Comments
-- =====================================================
COMMENT ON TABLE knowledge.indexed_documents IS 
    'Track metadata about analytics posts that have been indexed in the Qdrant vector database';

COMMENT ON COLUMN knowledge.indexed_documents.analytics_id IS 
    'Foreign key to analytics.post_analytics.id - unique identifier for the analytics post';

COMMENT ON COLUMN knowledge.indexed_documents.qdrant_point_id IS 
    'The vector point ID in Qdrant (usually matches analytics_id)';

COMMENT ON COLUMN knowledge.indexed_documents.content_hash IS 
    'SHA-256 hash of the document content for duplicate detection across different sources';

COMMENT ON COLUMN knowledge.indexed_documents.status IS 
